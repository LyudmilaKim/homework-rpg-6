...
public class Main {
    public static void main(String[] args) {
        ...
        ArenaFighter demoHero = new ArenaFighter("Valerian", 110, 0.25, 30, 6, 20, 4);
        ArenaOpponent demoOpponent = new ArenaOpponent("Champion", 95, 15);

        ActionQueue queue = new ActionQueue();
        queue.enqueue(new AttackCommand(demoOpponent, demoHero.getAttackPower()));
        queue.enqueue(new HealCommand(demoHero, 25));
        queue.enqueue(new DefendCommand(demoHero, 0.20));
        ...
        DefenseHandler armor = new ArmorHandler(7);
        DefenseHandler hp = new HpHandler();
        dodge.setNext(block).setNext(armor).setNext(hp);

        System.out.println("Sending 25 incoming damage through the defense chain...");
        System.out.println("Hero HP before: " + demoHero.getHealth());
        dodge.handle(25, demoHero);
        System.out.println("Hero HP after:  " + demoHero.getHealth());

        System.out.println("\n--- Full Arena Tournament ---");

        ArenaFighter tournamentHero = new ArenaFighter("Valerian", 150, 0.32, 40, 10, 26, 5);
        ArenaOpponent tournamentOpponent = new ArenaOpponent("Iron Vane", 140, 20);
