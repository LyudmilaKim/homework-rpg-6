...
public class TournamentEngine {
    ...
    private java.util.Random random = new java.util.Random(1L);
    ...
    public TournamentResult runTournament() {
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds) {
            ...
            actionQueue.enqueue(new HealCommand(hero, 25));      // ← изменено на 25
            actionQueue.enqueue(new DefendCommand(hero, 0.20));  // ← изменено на 0.20
            ...
            if (opponent.isAlive()) {
                System.out.println("[Round " + round + "] Opponent attacks with " + opponent.getAttackPower() + " damage!");
